...
/**
 * Standard plugin entry points of the quiz solution report.
 *
 ...
 */
function quiz_solution_question_preview_pluginfile($previewcontext, $questionid,
...
/**
 * Quiz solution report cron code. Deletes cached data more than a certain age.
 */
function quiz_solution_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quiz solution cache records...", '');

    $expiretime = time() - 5 * HOURSECS;
    $DB->delete_records_select('quiz_solution', 'timemodified < ?', array($expiretime));
